...
#### 5. For Post-Reboot / Boot Issues: Check Mounts and Filesystem Roots

**WHEN the symptom is "X is missing after reboot"** (project dir, config, data):

Add these checks before forming hypotheses:
- `lsblk` and `mount` to verify expected devices are mounted at expected points
- `find / -maxdepth N -name 'X' 2>/dev/null | grep -v proc | grep -v sys` to search all mountpoints for the missing item
- `stat /etc/fstab` or check timestamps to detect accidental reinstall/image replacement
- Check if the missing item lived on a partition that failed to mount or on a separate disk that was disconnected

**Why:** /home on BTRFS, separate data disks, and dual-boot setups commonly lose access to project dirs without explicit mount failures. The missing item may exist on an unmounted partition rather than being deleted.
