...
metadata:
  origin: ECC
  source: LobeHub affaan-m-everything-claude-code-git-workflow v1.0.3
  github: https://github.com/affaan-m/everything-claude-code/blob/main/skills/git-workflow/SKILL.md
---
...
If conflicts occur, Git will show conflicted files. Abort a test merge with `git merge --abort` if you were only checking.
...
Also communicate with the team about shared files, use feature flags instead of long-lived branches, and review/merge PRs promptly.
...
# BAD: "update" / "fix" commit messages
# GOOD: fix(auth): resolve redirect loop after login
...
# BAD: Committing generated files such as dist/ or node_modules/
...
1. Inspect the repository's current branch, remotes, and uncommitted changes before making workflow recommendations.
2. Prefer GitHub Flow unless the repository clearly needs trunk-based or GitFlow.
3. Use Conventional Commits for commit/PR titles.
4. Never run credential setup, remote pushes, branch deletion, force-push, tag publishing, or marketplace rating/commenting without explicit user confirmation.
5. Verify the final state with `git status`, relevant logs/diffs, and the requested CI/test command when available.
