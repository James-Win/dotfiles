...
metadata:
  origin: ECC
  source: LobeHub affaan-m-ecc-hermes-imports v1.0.3
  github: https://github.com/affaan-m/ECC/blob/main/skills/hermes-imports/SKILL.md
---
...
Do not export or publish private Hermes state unless the user explicitly asks and the content is sanitized. Never include secrets, tokens, OAuth files, private logs, account identifiers, contact graphs, finance/health/CRM data, or machine-specific paths in a public artifact.

Ask for confirmation before:
- Opening PRs or publishing a release pack
- Uploading files to any external service
- Using credentials or accessing private accounts
- Exporting local session, memory, or tool-output logs
- Making irreversible changes to a repository
...
- absolute paths such as `/Users/...`, `/home/<name>/...`, or platform-specific private paths
...
- active hostnames, LAN IPs, passwords, or service URLs that should stay private
...
- [ ] No secrets, credentials, OAuth files, or bearer strings included
- [ ] No private local paths unless required for local setup documentation
- [ ] No private logs, contact graphs, finance, health, CRM, or account data
- [ ] Local account names replaced with role labels
- [ ] Credential needs described by provider only
- [ ] Output contract included
- [ ] User confirmation obtained before external publication or PR actions
