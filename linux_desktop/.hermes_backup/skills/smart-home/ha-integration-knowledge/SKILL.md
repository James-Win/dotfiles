...
metadata:
  source: LobeHub home-assistant-core-integrations v1.0.2
  github: https://github.com/home-assistant/core/tree/dev/.claude/skills/ha-integration-knowledge
---
...
- **Diagnostics**: [`references/platform-diagnostics.md`](references/platform-diagnostics.md) for diagnostic data collection
- **Repairs**: [`references/platform-repairs.md`](references/platform-repairs.md) for user-actionable repair issues
...
When using this skill for a Home Assistant Core integration task:
1. Inspect the integration domain under `homeassistant/components/<domain>/` and tests under `tests/components/<domain>/`.
2. Read `manifest.json` and `quality_scale.yaml` first to understand declared quality level and required checks.
3. Prefer examples from platinum/gold integrations when borrowing patterns.
4. Keep integration code thin; push protocol/domain logic into upstream libraries instead of Home Assistant integration code.
5. Avoid adding user-configurable polling intervals or config-entry names.
6. Verify with appropriate Home Assistant tests and quality-scale rules before declaring completion.
7. Do not access credentials, real Home Assistant instances, external services, or device accounts without explicit user confirmation.
